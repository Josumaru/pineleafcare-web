    ...
    >
      ...
      <div className="mt-2">
        <p className="text-gray-400 text-sm">
          {data.date}・Bacaan {Math.round((data.content.length * 10) / 60)} Menit
        </p>
        <p className="text-white text-2xl my-2 font-bold">{data.title}</p>
        ...
        <div className="flex items-center gap-2">
          ...
          />
          <p className="text-xl font-bold text-white">{data.author.name}</p>
        </div>
      </div>
    </div>
  );
