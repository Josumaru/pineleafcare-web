...
//     formData.append("lat", `${m.lat}` );
//     formData.append("lng", `${m.lng}` );
//     formData.append("desc", m.desc ?? "");
//     formData.append("loc", `https://maps.app.goo.gl/${m.loc}` ?? "");
//     formData.append("province", m.province ?? "");
//     formData.append("city", m.city ?? "");
...
const city = [
  ...
  {
    city: ["Semarang", "Surakarta", "Kendal", "Salatiga", "Karanganyar"],
...
const MitraMap = () => {
  ...
  const filteredCity = selectedProvince
    ? city.find((item) => item.province === selectedProvince)?.city || []
    : [];
      ...
      if (!mapRef.current) {
        ...
        if (mapContainer) {
          ...
          filteredMarkers.forEach((marker) => {
            const googleMapsLink = marker.loc;
            if (mapRef.current) {
  ...
  useEffect(() => {
    ...
    const fetchMap = async () => {
      // Mengambil data marker
      const response = await fetch("/api/marker");
      if (response.ok) {
        const data = await response.json();
        // get province data from db
        let cityList: any[] = [];
        const provincesData = await data.data.map((item: any) => {
          return item.province;
        });
        const uniqueProvince = await provincesData
          .sort()
          .filter(
            (item: any, pos: number, ary: any) => !pos || item != ary[pos - 1]
          );
        await uniqueProvince.map((item: any) => {
          cityList.push({ province: item, city: [] });
        });
        ...
        await data.data.map((item: any) => {
          for (let i = 0; i < uniqueProvince.length; i++) {
            if (item.province == uniqueProvince[i]) {
              if (!cityList[i].city.includes(item.city)) {
                cityList[i].city.push(item.city);
              }
            }
          }
        });
  ...
  if (!isClient) {
    return (
      <div className="h-[400px] w-full px-2 flex items-center justify-center">
        <svg
          aria-hidden="true"
          role="status"
  ...
  return (
    <div className="px-2">
      ...
      <div
        id="map"
        key={refreshMap ? "active" : "inactive"}
        style={{ height: "400px", width: "100%" }}
        className="rounded-xl mt-5 z-10"
      />
      <div>
        <div className="mt-5 flex items-center space-x-4">
          <button
            className="px-10 py-2 rounded-lg text-black bg-white"
            onClick={handleReset}
          >
            Semua{" "}
          </button>
        ...
        {!showCards && (
          <div className="mt-8 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 ">
            {Object.values(markers).map((card, index) => {
              if (
                markers[index].city === selectedCity ||
                markers[index].province === selectedProvince
              ) {
                return (
                  <div key={index} className="bg-gray-500 rounded-lg p-4">
